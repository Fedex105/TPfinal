    ...
    filtroDestacado(prod){
     ...
     renderProd(){
        ...
        productos = productos.slice(0,4)
            return productos.map(producto => <ProductoIndividual id={producto.idProductos} nombre={producto.nombre} precio={producto.precio} imagenes={producto.imagenes[0]}/>)
    }
...
class ListadoProductos extends React.Component{
    ...
    filtroEnvio(prod){
        return prod.seEnvia === 1;
    }
    filtroStock(prod){
        return prod.stock > 0;
    }
    filtroInsumo(prod){
        return prod.categoria === "Insumos";
    }
    filtroMateriales(prod){
        return prod.categoria === "Materiales";
    }
    filtroCervezas(prod){
